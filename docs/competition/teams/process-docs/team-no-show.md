...
* Marking them as `dropped_out_after` the last match of the previous stage of the competition -- probaly the Virtual League if there was one, or match `-1` if there are no prior matches
...
Repeat this at 11:00 on Saturday for any teams still not present.

At 11:15 (TODO: what would they be doing at that point?) provide an update to the Head Shepherds regarding which teams are present or not -- at this point teams may be in one of three states: present, AWOL, dropped out. The Head Shepherds can cascade this down to their teams ahead of the matches.

After 11am Saturday, Reception should actively inform the person running this process of new arrivals as they arrive. Reception should also inform the Head Shepherd.
TODO: Are the people doing the shepherding the same as the ones who were guiding teams to their pits? If so, who guides the team to their pits if they're late?

@Alexbruvv (as you've been working on reception stuff) and @samjmartin04 (as Match Ops), thoughts on this?

TODO: Sunday would be broadly similar, though with different timings and wouldn't touch the league schedule.
